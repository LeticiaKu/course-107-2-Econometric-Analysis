...
journals %>% 
  ...
  pairs.panels()
#psych::pairs.panels用來畫圖
(dataframe )
```

```{r}
#traditional usage (data,想選的變數)
select(journals,citeprice,subs) -> tempDF
#traditioanal pairs.panels
pairs.panels(tempDF)
#pipe寫法
journals %>% 
  select(citeprice,subs) ->
...
coeftest(model_2, vcov. = vcovHC, type="HC1") -> model_2_coeftest  #此物件為矩陣
```

standard error 原始作法，使用矩陣(必須下asis 不然會顯示網路語言 而不是表格)
```{r results="asis"}
stargazer(model_1, model_2, 
          se=list(model_1_coeftest[,"Std. Error"], model_2_coeftest[,2]),
          type="html",
          align=TRUE)  #align資料對齊
```
